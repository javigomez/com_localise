...
class InstallTest extends JoomlaWebdriverTestCase
{
    ...
    public function install_NormalInstallFromConfig_ShouldInstall()
    {
       ...
       //$cpPage->clearInstallMessages();
        ...
        $extensionInstallerPage = $cpPage->clickMenu('Extension Manager', 'ExtensionManagerPage');
        $extensionInstallerPage->installComLocalise($this->cfg);
        $this->assertTrue($extensionInstallerPage->verifyInstallation($this->cfg,'Localise'), 'Extension must be installed');
        $this->doAdminLogout();
    }
